...
const EXPECTED_PACKAGE_FILES = [
  ...
  "dist/src/runtime/decimals.js",
  "dist/src/runtime/decimals.d.ts",
  "dist/src/runtime/routeAccess.js",
  "dist/src/runtime/react/default-ui/index.js",
  "src/routes/dashboard.astro",
  "src/routes/processes.astro",
  "src/routes/schools.astro",
  "src/scaffold/styles/reparto.css"
];

const EXPECTED_REGISTRY_ITEMS = [
  "registry.json",
  "reparto-crud-table.json",
  "reparto-fk-select.json",
  "reparto-processes-table.json",
  "reparto-starter-views.json",
  "reparto-state-panel.json"
];
...
function verifyExports(installedRoot) {
  ...
  for (const subpath of EXPECTED_EXPORT_SUBPATHS) {
    ...
    const specifier = subpath === "." ? "@mano8/astro-reparto-m8" : `@mano8/astro-reparto-m8/${subpath.slice(2)}`;
  ...
  assert(
    packageJson.repartoDocenteM8?.contract === "reparto-docente-m8@2.0.0",
    `unexpected published contract: ${packageJson.repartoDocenteM8?.contract}`
  );
}
...
function main() {
  ...
  const installedRoot = join(WORK_DIR, "node_modules", "@mano8", "astro-reparto-m8");
